...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){const $lb=$(`#${lightboxId}`);$lb.find(".lightboxImage").attr("src",element.attr("src"));if(window.bootstrap&&window.bootstrap.Modal){const modalEl=document.getElementById(lightboxId);const modal=window.bootstrap.Modal.getOrCreateInstance(modalEl);modal.show();return}},prevImage(lightboxId){const $lb=$(`#${lightboxId || "galleryLightbox"}`);const currentSrc=$lb.find(".lightboxImage").attr("src");const activeTag=$(".tags-bar span.active-tag").data("images-toggle");let $images=$("img.gallery-item");if(activeTag&&activeTag!=="all"){$images=$images.filter(function(){return $(this).data("gallery-tag")===activeTag})}
const images=$images.toArray();if(!images.length)return;let index=images.findIndex(img=>img.getAttribute("src")===currentSrc);if(index===-1)index=0;const prevIndex=(index-1+images.length)%images.length;$lb.find(".lightboxImage").attr("src",images[prevIndex].getAttribute("src"))},nextImage(lightboxId){const $lb=$(`#${lightboxId || "galleryLightbox"}`);const currentSrc=$lb.find(".lightboxImage").attr("src");const activeTag=$(".tags-bar span.active-tag").data("images-toggle");let $images=$("img.gallery-item");if(activeTag&&activeTag!=="all"){$images=$images.filter(function(){return $(this).data("gallery-tag")===activeTag})}
const images=$images.toArray();if(!images.length)return;let index=images.findIndex(img=>img.getAttribute("src")===currentSrc);if(index===-1)index=0;const nextIndex=(index+1)%images.length;$lb.find(".lightboxImage").attr("src",images[nextIndex].getAttribute("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
